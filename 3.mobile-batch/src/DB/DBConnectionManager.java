...
public class DBConnectionManager {
	 ...
	 *
	 */
    static public DBConnectionManager getCurrInstance() {
		return instance;
	}
    ...
    private void init() {
		...
		finally
		{
        	try {
            	...
            	System.err.println("Can't close the properties file. " + e);
			}
		}
    ...
    class DBConnectionPool {
        ...
        public synchronized Connection getConnection() {
            ...
            if (freeConnections.size() > 0) {
     		   ...
     		   /*---------------------------------------------------------------------
    			* add
    			* 간혹 DB 와 Connection 이 단절되는 경우가 생긴다.
    			* 이에 보완 대책이다.
    			* --- DB 단절의 이유 (가정) ---
    			* Network 상의 장애일 수도 있고 DB의 Network Listener 가 오동작을
    			* 일으켜서 일 수도 있다.
    			----------------------------------------------------------------------*/
               	log("maxConn[" + maxConn + "]");
                while((con = newConnection()) == null)
				{
    				DBConnectionManager.getCurrInstance().release();
    				DBConnectionManager.getCurrInstance().init();
				}
            }
            if (con != null) {
                checkedOut++;
